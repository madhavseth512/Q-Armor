...
accuracy but reverses ranking metrics. Item~(2) is now enforced
structurally: every reported result in this paper, including the
episode-evaluation harness of Section~\ref{subsec:reflexive}, computes
hard predictions via the native \texttt{predict\_labels()} margin
decision rather than \texttt{argmax(predict\_proba())} -- a discrepancy
we discovered while regenerating Table~\ref{tab:episodes}, where the
uncorrected path produced a hard prediction of ``attack'' for every
sample in an episode regardless of the underlying AUROC. Items~(1),~(3),
and~(4) remain open and are listed in
Table~\ref{tab:protocol_status}.
...
Label $\to$ internal $\pm1$ mapping verified & $\sim$ \\
\texttt{predict} vs.\ sign(\texttt{decision\_function}) test & \ding{55} \\
Manual toy-example AUROC agreement check & \ding{55} \\
\midrule
