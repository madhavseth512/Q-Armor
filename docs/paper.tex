...
controls adaptation samples can induce persistent model changes.
Section~\ref{subsec:recurpoison} measures this exposure directly:
\textsc{Reinforce} fires on a poisoned-pool model at every contamination
rate tested, confirming the limitation empirically rather than only
structurally. Robust adaptation under contaminated target pools remains
future work.
...
(116/116 tests pass); none affects a headline number in this paper, since
...
\begin{table}[t]
\centering
\caption{10-episode reflexive-control trace. ``Drift'' denotes ADWIN
firing on the supervised error stream. Binary F1 is omitted because it is
constant at $0.667$ across every QSVC episode and therefore
uninformative here -- see the discussion below.}
\label{tab:episodes}
...
Binary F1 is omitted from Table~\ref{tab:episodes} because it is not
informative for the QSVC episodes: it is constant at $0.667$
regardless of AUROC, the
...
retraining recovers $0.307$ AUROC in the reported run. Binary F1 does
\emph{not} improve: it falls slightly, from $0.440$ to $0.398$
($-0.042$), because adaptation shifts where the hard-decision boundary
sits relative to the episode's class balance rather than making the
decision proportionally more accurate -- the same instability discussed
in Section~\ref{subsec:episode_results}. Stage~2 typing does not
recover.
...
\caption{Stage-1 AUROC and FPR@TPR95 for the held-out evaluation
...
For real-hardware characterisation, we ran a 24-point kernel computation
(276 unique off-diagonal entries; not a full train/evaluate cycle) on
...
$113\,\text{s}$, of which $77\,\text{s}$ was billed quantum-processor
time -- $12.8\%$ of the Open plan's 600-second (10-minute) 28-day
allowance for the entire real-QPU characterisation requirement of
Section~\ref{subsec:confirmatory}.

Mean absolute kernel error against the exact simulator was $0.0264$
(95\% bootstrap CI $[0.0204,\,0.0326]$), but the error distribution is
strongly skewed (median $0.0018$, p95 $0.153$, max $0.195$) and its
structure is systematic rather than random. Hardware entries are biased
\emph{downward} -- mean signed deviation $-0.024$, with $68\%$ of entries
below their ideal value -- and the deviation scales with the overlap
being measured: mean $|\Delta|$ rises monotonically from $0.0012$ for
near-orthogonal pairs ($K<0.01$, $n{=}145$), through $0.026$ for
$0.1\le K<0.5$, to $0.154$ for near-identical pairs ($K\ge0.9$,
$n{=}17$). This is the signature of decoherence pulling large overlaps
toward the maximally mixed state while leaving near-orthogonal pairs
essentially exact. The induced similarity \emph{ordering} nonetheless
survives (Spearman $\rho=0.957$, Pearson $r=0.999$ against the ideal
kernel), so the hardware Gram matrix remains structurally usable even
though individual high-overlap entries are materially depressed.

One correction is warranted: an earlier 4-point (6-entry) version of this
experiment reported a mean error of $0.009$. That sample contained
essentially no high-overlap pairs (largest ideal entry $0.298$) and so
under-sampled exactly the regime in which hardware noise dominates. The
276-entry figures above supersede it.
...
(v)~real-hardware evidence is a 276-entry kernel characterisation
