...
the LLM-pilot sub-items of Table~\ref{tab:protocol_status}, are the
protocol's remaining scope.
...
Multi-seed repetition (10+ seeds, bootstrap CI) & $\sim$ (10 classical; 5 quantum, Sec.~\ref{subsec:budgetsweep}) \\
Equal-budget classical baselines (linear SVM, RBF-SVM, RF, GBT) & \ding{51} \\
Budget sweep $B\in\{0,25,50,100,150,300\}$ & \ding{51} (Sec.~\ref{subsec:budgetsweep}) \\
Natural class-prevalence evaluation & \ding{51} (Sec.~\ref{subsec:naturalprev}) \\
Temporal / capture-aware splits & \ding{55} (infeasible: no timestamp field in either dataset) \\
Additional source$\to$target pairs & \ding{55} \\
Kernel-target alignment / effective rank & \ding{51} (Sec.~\ref{subsec:kerneldiag}) \\
Finite-shot sweep $S\in\{128,\dots,4096\}$ & \ding{51} (Sec.~\ref{subsec:finiteshot}) \\
Real-QPU characterisation & \ding{51} (minimal 6-pair run, \texttt{ibm\_fez}; Sec.~\ref{subsec:kerneldiag}) \\
Recurring A$\to$B$\to$A drift study & \ding{51} (Sec.~\ref{subsec:recurpoison}) \\
Adaptation-pool poisoning study & \ding{51} (Sec.~\ref{subsec:recurpoison}) \\
\midrule
...
finite-shot pilot required by Table~\ref{tab:protocol_status}; it is
still not QPU validation or evidence of robustness to hardware noise.
...
report the required extra metrics: precision, MCC, balanced accuracy,
TPR at FPR${=}0.01$, 10-bin expected calibration error (ECE), and
...
Table~\ref{tab:protocol_status}. Mean absolute kernel error against the
...
fixed 2-episode window; Table~\ref{tab:protocol_status} still lists a
multi-seed reflector comparison as open, so this remains a motivated
design distinction rather than a demonstrated one.
...
\textsc{Reinforce} fired at \emph{every} poison rate for \emph{every}
...
protocol's open items (Table~\ref{tab:protocol_status}).
